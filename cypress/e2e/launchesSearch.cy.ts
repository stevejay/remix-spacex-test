describe("launches search tests", () => {
  ...
  it("displays the search results in a table", () => {
    ...
    cy.findByRole("table")
      ...
      .should("exist");
    cy.findByRole("table")
      .findByRole("button", { name: "Date" })
      .should("exist");
    cy.findByRole("table")
      .findByTestId("5fe3af84b3467846b3242161") // ID for 'CRS-22 & IROSA' launch
      .within(($row) => {
  ...
  it("allows the user to change the sorting field and sorting direction of the results table", () => {
    ...
    cy.findByRole("table").should("exist");
    cy.findByRole("table")
      .findByRole("columnheader", { name: "Name" })
      .should("have.attr", "aria-sort", "none");
    cy.findByRole("table")
      .findByRole("columnheader", { name: "Date" })
      .should("have.attr", "aria-sort", "descending");
    cy.url().should("include", "sortAscending=false");
    cy.url().should("include", "sortField=date_utc");
    cy.url().should("include", "name=star");

    cy.findByRole("table").findByRole("button", { name: "Date" }).click();
    cy.findByRole("table")
      .findByRole("columnheader", { name: "Name" })
      .should("have.attr", "aria-sort", "none");
    cy.findByRole("table")
    ...
    cy.url().should("include", "sortAscending=true");
    cy.url().should("include", "sortField=date_utc");
    cy.url().should("include", "name=star");

    cy.findByRole("table").findByRole("button", { name: "Name" }).click();
    cy.findByRole("table")
      .findByRole("columnheader", { name: "Name" })
      .should("have.attr", "aria-sort", "ascending");
    cy.findByRole("table")
      ...
      .should("have.attr", "aria-sort", "none");
    cy.url().should("include", "sortAscending=true");
    cy.url().should("include", "sortField=name");
    cy.url().should("include", "name=star");
